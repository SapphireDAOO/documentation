...
A seller is a participant who provides goods or services and receives payment through the system. Invoice interaction differs by processor: in the simple processor the seller creates the invoice, after the buyer **makes payment**, the seller accepts or rejects the payment; in the intermediated processor, the system assigns it to the seller, then the payment-flow actions on the seller's behalf depend on the integrating third-party platform.
...
### Intermediated Platforms Operator Wallet

The Intermediated Payment Processor contract designates an authorized platform operator wallet address that oversees invoice creation and resolution workflows. Only the platform operator wallet can perform sensitive actions such as releasing payments, resolving disputes, and creating single or meta-invoices.
